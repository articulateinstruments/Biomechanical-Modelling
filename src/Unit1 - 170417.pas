...
//  il:=muscle[i].TrackBar.Position;
    ...
    begin
      ...
      begin
        TargLen[il]:=max(R*muscle[musc].TrackBar.Position/100,1);  // TargLen = threshold strut length at which no force applies
        PrevTargLen[il]:=max(R*muscle[musc].TrackBar.selEnd/100,1);  // TargLen = threshold strut length at which no force applies
...
//        begin
          if (prevlen-d1 >= 0) and (prevlen -Targlen[il] > 0) then
            v:=(prevlen-d1)/(prevlen-Targlen[il])
          else v:=0;
//        end else
//          v:= 0;
          v:=1-v;
...
//          Elasticity[il] := (prevlength-TargLen[il])/restlength;;                      //
          ...
          vect2:=Scale3D(vect2,ElasticityC*Elasticity[il]);     //    force = stiffness constant *(L-l)(L-l)
          R:=restlength;
          dL:=Length3D(vect2);
          prevLen:=sqrt(dL/sqrt(ElasticityC))/dL;
//          af:=0;
//          if musc >= 0 then
