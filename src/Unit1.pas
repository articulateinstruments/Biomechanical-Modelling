...
type
  ...
  Tmuscle = record
    ...
    EMA: record
      n: integer;
      y,z,phi,theta,rms,extra: single;
    end;
  end;
  TRoof = record
    h: single;
...
//    procedure DeleteAllMuscles1Click(Sender: TObject);
    ...
    procedure EMA2Click(Sender: TObject);
    procedure MoveBGImage1Click(Sender: TObject);
  private
    ...
    procedure StringToAtoms(s: string; LoadOptions,LoadCaption,RetainKeyframes,LoadBGshift: boolean);
...
uses
  ...
  EMAdlg,
  pngimage, ImportNodesDlg, BGShiftDlg;
...
//      procedure Periphery(force: T3dPoint; forceScale,pressure,PressureMax: single);
      procedure Periphery(force: T3dPoint; forceScale,Volume,InitVolume: single);
      var i: integer;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Brush.Color:=RGB(
              round((Red  (ch)-Red  (cl))*RealRange((Volume-InitVolume/2)/(InitVolume),0,1)+Red  (cl)),
              round((Green(ch)-Green(cl))*RealRange((Volume-InitVolume/2)/(InitVolume),0,1)+Green(cl)),
              round((Blue (ch)-Blue (cl))*RealRange((Volume-InitVolume/2)/(InitVolume),0,1)+Blue (cl)));
...
//          Periphery(force,ForcesScale,Polyhedron[ph].pressure,PressureMax);
          Periphery(force,ForcesScale,PolyhedronVolume(ph),Polyhedron[ph].volume);
        end;
      end;
    end;
...
//          FillPolyhedra(false);
  ...
  begin
    ...
    begin
      x1:=round(zoom*bmp.width*dlgBGShift.fsSize.Value/100);
      y1:=round(zoom*bmp.height*dlgBGShift.fsSize.Value/100);
...
begin
  ...
  pnlMuscleChart.Height:=LoadSaveMyIniFileInt(Load,'General','pnlMuscleChart.Height',pnlMuscleChart.Height,pnlMuscleChart.Height);
  for RoofAxes:=low(RoofAxes) to high(RoofAxes) do
   ...
   if running then
    ...
    with Struts[il] do
      if musc >= 0 then    //if strut assigned to a muscle then
        PrevTargLen[il]:=max(restlength*muscle[musc].TrackBar.position/100,1);  // TargLen = threshold strut length at which no force applies
...
//  sbCentreClick(Sender);
  if Running then              //added aaw 080124
    Timer1.enabled:=true
  else Timer1.enabled:=false;  //added
...
//    Ofs2DZ:=LoadSaveMyIniFileInt(true,'Layout','RunOfs2DZ',Ofs2DZ,Ofs2DZ);
  ...
  begin
    ...
    pnlMuscles1.Enabled:= false;
    if SavedPosition <> '' then
      StringToAtoms(SavedPosition,false,true,true,false);
...
begin
  ...
  begin
    if  TrackBar.position<>i then
      for il:=1 to NumStruts do
        with Struts[il] do
          if musc = m then    //if strut assigned to a muscle then
            PrevTargLen[il]:=max(restlength*muscle[musc].TrackBar.position/100,1);  // TargLen = threshold strut length at which no force applies
    if TrackBar.position>i then
    begin
...
begin
  try
    ...
    WriteRoof;
    if dlgEMA <> nil then
      AddToResult(dlgEMA.WriteEMA);
    if dlgBGShift <> nil then
      AddToResult(dlgBGShift.BGShiftToString);
...
procedure TForm1.StringToAtoms(s: string; LoadOptions,LoadCaption,RetainKeyframes,LoadBGShift: boolean);
...
coeff:=1;
      ...
      begin
        ...
        with muscle[m] do
        begin
          muscle[m].chart:=NextNumber(line) <> 0; Mustbe(line,',');
    ...
    begin
      ...
      try
        ...
        case line[1] of
          ...
          'H': LoadPolyhedron;
          'Y': LoadSymmetry;
          'E': dlgEMA.LoadEMA(line);
          'B': if LoadBGShift then dlgBGShift.StringToBGShift(line);
        end;
      except
  ...
  begin
    try
      try
        ...
        pnlMuscleChart.Visible:=false;
        for m:=0 to high(muscle) do
          if muscle[m].Chart then
            pnlMuscleChart.Visible:=true;
        Splitter1.Visible:=pnlMuscleChart.Visible;
...
begin
  if Undo.Count > 0 then
    StringToAtoms(Undo[Undo.Count-1],true,true,false,true);
...
//      HideAllExcept:=StringToTName('');
      result:=false;
      StringToAtoms(LoadFromFileString(FileName),true,true,false,true);
      StopAllAtoms(false);
      result:=true;
    except
...
procedure TForm1.Timer1Timer(Sender: TObject);          //Iteration clock = 12ms
...
var TotalVolumeC,TotalVolumeT,TotalForceT,{GasConst,GasConstArea,ClipGas,}expansion: single;        //moved to global scope as spinedits and comboboxes are slow o access
...
var pass,i,j,m,ia,il,it,t: integer;
    s,b,v,dArea,d1,R,af,dL: Single;
    vect, vect1,vect2: T3DPoint;
    wgt: TAtomRealArray;
    cnt: TAtomIntArray;
    th,ph,pg: integer;
    Elasticity,TargLen: TStrutRealArray;
begin
//  cbf:=Tcbf(dlgOptions.cbForceKind.ItemIndex);      //moved to global variables
...
//    ApplyRigity;
    ...
    begin
      ...
      if musc >= 0 then
      begin
        TargLen[il]:=max(R*muscle[musc].TrackBar.Position/100,1);
//        TargLen[il]:=(TargLen[il]-R)*Coeff+R;
        Elasticity[il]:=(ElasticityR-ElasticityC)*TargLen[il]/R+ElasticityC;
      end else
      begin
        TargLen[il]:=R;
        TargLen[il]:=max(R,1);
...
//      d1:=Length3D(vect);
      ...
      begin
        ...
        begin
          b:=b/2;
          vect:=Scale3D(vect,b);  //this means the longer the strut the more force it applies which is wrong
//          vect:=Scale3D(vect,b*40/d1);  //40 is an approximate average strut length
...
//          b:=b/2;
//          vect:=Scale3D(vect,b*b/d1);  //this scales the vector to the square of the the length difference multiplied by the stiffnesss
//          Atoms[Atom1].f:=Sub3D(Atoms[Atom1].f, Scale3D(vect,Elasticity[il])); wgt[Atom1]:=wgt[Atom1]+Elasticity[il];
//          Atoms[Atom2].f:=Add3D(Atoms[Atom2].f, Scale3D(vect,Elasticity[il])); wgt[Atom2]:=wgt[Atom2]+Elasticity[il];
        end;
          if cbf = cbfStrutElastic then
            Atoms[Atom1].af:=Atoms[Atom1].f;
          if cbf = cbfStrutElastic then
            Atoms[Atom2].af:=Atoms[Atom2].f;
      end;
{    NEW
    for il:=1 to NumStruts do
    with Struts[il] do
    begin
      if  Atoms[Atom1].p.y > 10000 then
        vect1 :=  Atoms[Atom1].p;
      ...
      begin
        ...
        begin
          ...
          vect2:=Scale3D(vect2,Elasticity[il]);     //    force = stiffness constant (ElasticityC) *(l-L)(l-L)
          R:=restlength;
          prevLen:=d1;
          Atoms[Atom1].f:=Sub3D(Atoms[Atom1].f, vect2);      //Add the negative force vector to atom1
          if cbf = cbfStrutElastic then
            Atoms[Atom1].af:=Sub3D(Atoms[Atom1].af, vect2);
          wgt[Atom1]:=wgt[Atom1]+ElasticityC/2;             // half the force for each atom      wgt is accumulation of ElasticityC values for every strut attached to the atom
      ...
      {for EMA ----------------}
      if pass = dlgOptions.seSolve.value then
      if musc >= 0 then
      with muscle[musc] do
      begin
        inc(EMA.n);
        ...
        begin
          EMA.y:=EMA.y+(Length3D(vect)-TargLen[il]) / restlength;
          EMA.z:=EMA.z+sqr(Length3D(vect)-TargLen[il]) / restlength;
        end;
        EMA.phi:= 0;//EMA.phi+Length3D(vect2);
        EMA.theta:=EMA.theta+Length3D(vect);
        EMA.rms:=EMA.rms+TargLen[il];
        EMA.extra:=EMA.extra+restlength;
      end;
{   NEW
      if pass = dlgOptions.seSolve.value then
      if musc >= 0 then
      ...
      begin
        ...
        if restlength > 0 then
        begin
          EMA.y:=EMA.y+(Length3D(vect1)-TargLen[il]) / restlength;
          EMA.z:=EMA.z+sqr(Length3D(vect1)-TargLen[il]) / restlength;
        end;
        EMA.phi:=EMA.phi+Length3D(vect2);
        EMA.theta:=EMA.theta+Length3D(vect1);
        EMA.rms:=EMA.rms+TargLen[il];
        EMA.extra:=EMA.extra+restlength;
      end;
}
    ...
    begin
      ...
      if cbf = cbfStrutElastic then
        Atoms[ia].af:=Scale3D(Atoms[ia].f,1/wgt[ia]);
    end; 
...
{    NEW
    ...
    if wgt[ia] <> 0 then
    begin
      dL := Length3D(Atoms[ia].f);       //dL is magnitude of force acting on atom
      if (dL <> 0) then                                             // force=k(L-l)(L-l)
        Atoms[ia].f:=Scale3D(Atoms[ia].f,(sqrt(dL/sqrt(wgt[ia]))/dL))      // root(F/root(k))/F
      else Atoms[ia].f:= Zero3DPoint;
      if cbf = cbfStrutElastic then
        if dL <>0 then
          Atoms[ia].af:=Scale3D(Atoms[ia].af,(sqrt(dL/sqrt(wgt[ia]))/dL))
        else Atoms[ia].f:= Zero3DPoint;
...
//    TotalForce := TotalForce+ force;
    ...
    begin
      inc(EMA.n);
      EMA.y:=EMA.y+(dLen-d2) / restlength;
      EMA.z:=EMA.z+sqr(dLen-d2) / restlength;
      EMA.phi:=EMA.phi+Length3D(force);
      EMA.theta:=EMA.theta+dLen;
      EMA.rms:=EMA.rms+d2;
      EMA.extra:=EMA.extra+restlength;
    end;
    ...
    WM_KEYDOWN:
      ...
      begin
        case Msg.wParam of
          ...
          vk_MakeStrut: begin SetCursor; Handled:=True; End;     //setcursor changes curson icon
...
begin
  ...
  Chart1.enabled:=(puMuscles.PopupComponent <> pnlMuscles1);

  if puMuscles.PopupComponent <> pnlMuscles1 then
  begin
    SetCurMuscle(puMuscles.PopupComponent.Tag);
    Chart1.Checked:=muscle[CurMuscle].Chart;
  end;
...
var m,k: integer;
const previousrunstate : boolean = false;
begin
CheckAllKeyframes;
  result:=-1;
  for k:=0 to high(Keyframe) do
  ...
  if (pnlMuscles1.Enabled <> (result >= 0)) or (previousrunstate <> running) then   //if at a keyframe or edit/run mode has changed
  begin
    ...
    Edit1.enabled:=(result >= 0) or not running;
    Selection1.enabled:=(result >= 0) or not running;
    Points1.enabled:=(result >= 0) or not running;
    Struts1.enabled:=(result >= 0) or not running;
    Polygon1.enabled:=(result >= 0) or not running;
    View1.enabled:=(result >= 0) or not running;
...
//    BevelOuter:=bvNone;
    ...
    muscle[high(muscle)].Chart:=false;
...
begin
  for k:=0 to high(Keyframe)-1 do
    assert(Keyframe[k].TLTime <= Keyframe[k+1].TLTime,'Keyframes not sorted');
end;
{
...
function TForm1.SectionRect(Axes: TAxes): TRect;
  procedure SetRect(ALeft, ATop, ARight, ABottom, aSize: Single);
  var w,h: Single;
  begin
    if dlgBGShift <> nil then
    begin
      w := (ARight-ALeft)*dlgBGShift.fsSize.Value/100;
      h := (ABottom-ATop)*dlgBGShift.fsSize.Value/100;
      ALeft:=ALeft+dlgBGShift.fsHShift.Value/100*aSize;
      ATop:=ATop+dlgBGShift.fsVShift.Value/100*aSize;
      ARight:=ALeft+w;
      ABottom:=ATop+h;
    end;
    result:=Rect(
...
begin
  ...
  begin
    ...
    case Axes of
      plXZ: SetRect((pbXZ.Width-dlgImportImages.Pixs[0].Width)/2,pbXZ.Height-dlgImportImages.Pixs[0].Height,(pbXZ.Width-dlgImportImages.Pixs[0].Width)/2+dlgImportImages.Pixs[0].Width,pbXZ.Height,dlgImportImages.Pixs[0].Width);
      plXY: SetRect((pbXY.Width-dlgImportImages.Pixs[0].Width)/2,(pbXY.Height-dlgImportImages.Pixs[0].Width)/2,(pbXY.Width-dlgImportImages.Pixs[0].Width)/2+dlgImportImages.Pixs[0].Width,(pbXY.Height-dlgImportImages.Pixs[0].Width)/2 + dlgImportImages.Pixs[0].Width,dlgImportImages.Pixs[0].Width);
      plYZ: SetRect((pbYZ.width-dlgImportImages.Pixs[0].Width)/2,pbYZ.Height-dlgImportImages.Pixs[0].Height,(pbYZ.width-dlgImportImages.Pixs[0].Width)/2+dlgImportImages.Pixs[0].Width,pbYZ.Height,dlgImportImages.Pixs[0].Width);
    end else
    case Axes of
      plXZ: if bmpXZ.Width > 0 then SetRect((pbXZ.Width-bmpXZ.Width)/2,pbXZ.Height-dlgImportImages.Pixs[0].Height,(pbXZ.Width-bmpXZ.Width)/2+bmpXZ.Width,pbXZ.Height,bmpXZ.Width);
      plXY: if bmpXY.Width > 0 then SetRect((pbXY.Width-bmpXY.Width)/2,(pbXY.Height-bmpXY.Height)/2,(pbXY.Width-bmpXY.Width)/2+bmpXY.Width,(pbXY.Height-bmpXY.Height)/2 + bmpXY.Height,bmpXY.Width);
      plYZ: if bmpYZ.Width > 0 then SetRect((pbYZ.width-bmpXY.Width)/2,pbYZ.Height-bmpXY.Height,(pbYZ.width-bmpXY.Width)/2+bmpXY.Width,pbYZ.Height,bmpYZ.Width);
    end;
  end else
  begin
    case Axes of
      plXZ: if bmpXZ.Width > 0 then
              SetRect((pbXZ.Width-bmpXZ.Width)/2,pbXZ.Height-bmpXZ.Height,(pbXZ.Width-bmpXZ.Width)/2+bmpXZ.Width,pbXZ.Height,bmpXZ.Width) else
              SetRect((pbXZ.Width-1000)/2,pbXZ.Height-1000,(pbXZ.Width-1000)/2+1000,pbXZ.Height,1000);
      plXY: if bmpXY.Width > 0 then
              SetRect((pbXY.Width-bmpXY.Width)/2,(pbXY.Height-bmpXY.Height)/2,(pbXY.Width-bmpXY.Width)/2+bmpXY.Width,(pbXY.Height-bmpXY.Height)/2 + bmpXY.Height,bmpXY.Width) else
              SetRect((pbXY.Width-1000)/2,(pbXY.Height-1000)/2,(pbXY.Width-1000)/2+1000,(pbXY.Height-1000)/2 + 1000,1000);

      plYZ: if bmpYZ.Width > 0 then
              SetRect((pbYZ.width-bmpYZ.Width)/2,pbYZ.Height-bmpYZ.Height,(pbYZ.width-bmpYZ.Width)/2+bmpYZ.Width,pbYZ.Height,bmpYZ.Width) else
              SetRect((pbYZ.width-1000)/2,pbYZ.Height-1000,(pbYZ.width-1000)/2+1000,pbYZ.Height,1000);
    end;
  end;
end;
  ...
  if dlgEMA <> nil then
    with dlgEMA do
      for i:=0 to ListBox1.Items.Count-1 do
        if InRange(loword(integer(ListBox1.Items.Objects[i])),1,Form1.NumAtoms-1) then
...
begin
  ...
  begin
    muscle[CurMuscle].Chart:=not muscle[CurMuscle].Chart;
    if muscle[CurMuscle].Chart then
    begin
      pnlMuscleChart.Visible:=true;
...
begin
  ...
  begin
    ...
    begin
      ...
      pen.color:=clBlack;
      Brush.Style:=bsClear;
      Textout(2,y1,before(muscle[m].Label1.Caption,'='));
      Brush.Style:=bsSolid;

      inc(y1,3);
      dec(y2,2);

      y:=y1+(y2-y1-1)*(Keyframe[0].posq[m].p-muscle[m].TrackBar.min) div (muscle[m].TrackBar.max-muscle[m].TrackBar.min);
      MoveTo(0,y);
      for k:=0 to high(Keyframe) do
        if Keyframe[k].posq[m].Significant then
        begin
          LineTo(TLTimeToX(Keyframe[K].TLTime)+pnlTimelineRed.Width div 2,y);
          y:=y1+(y2-y1-1)*(Keyframe[k].posq[m].p-muscle[m].TrackBar.min) div (muscle[m].TrackBar.max-muscle[m].TrackBar.min);
          LineTo(TLTimeToX(Keyframe[K].TLTime)+pnlTimelineRed.Width div 2,y);
        end;
      LineTo(width,y);
...
procedure TForm1.EMA2Click(Sender: TObject);
begin
  dlgEMA.ShowModal;
  InvalidateAll;
end;

procedure TForm1.MoveBGImage1Click(Sender: TObject);
begin
  dlgBGShift.Show;
end;
...
==============================================================================================
function TForm1.SectionRect(Axes: TAxes): TRect;
  procedure SetRect(ALeft, ATop, ARight, ABottom: Single);
  begin
    result:=Rect(
      Round(ALeft),
      Round(ATop),
...
begin
  ...
  case Axes of
    plXZ: begin pbW:=pbXZ.Width; pbH:=pbXZ.Height; bmW:=bmpXZ.Width; bmH:=bmpXZ.Height; end;
    plXY: begin pbW:=pbXY.Width; pbH:=pbXY.Height; bmW:=bmpXY.Width; bmH:=bmpXY.Height; end;
    plYZ: begin pbW:=pbYZ.Width; pbH:=pbYZ.Height; bmW:=bmpYZ.Width; bmH:=bmpYZ.Height; end;
  end;
  ...
  begin
    ...
    if (dlgImportImages <> nil) and (length(dlgImportImages.Pixs) > 0) then
    case Axes of
      plXZ: SetRect((pbW-dlgImportImages.Pixs[0].Width)/2, pbH-dlgImportImages.Pixs[0].Height,(pbW-dlgImportImages.Pixs[0].Width)/2+dlgImportImages.Pixs[0].Width,pbH);
      plXY: SetRect((pbW-dlgImportImages.Pixs[0].Width)/2,(pbH-dlgImportImages.Pixs[0].Width)/2,(pbW-dlgImportImages.Pixs[0].Width)/2+dlgImportImages.Pixs[0].Width,(pbH-dlgImportImages.Pixs[0].Width)/2 + dlgImportImages.Pixs[0].Width);
      plYZ: SetRect((pbw-dlgImportImages.Pixs[0].Width)/2, pbH-dlgImportImages.Pixs[0].Height,(pbw-dlgImportImages.Pixs[0].Width)/2+dlgImportImages.Pixs[0].Width,pbH);
    end else
    case Axes of
      plXZ: if bmW > 0 then SetRect((pbW-bmW)/2, pbH-dlgImportImages.Pixs[0].Height,(pbW-bmW)/2+bmW,pbH);
      plXY: if bmW > 0 then SetRect((pbW-bmW)/2,(pbH-bmH)/2,(pbW-bmW)/2+bmW,(pbH-bmH)/2 + bmH);
      plYZ: if bmW > 0 then SetRect((pbw-bmW)/2, pbH-bmH,(pbw-bmW)/2+bmW,pbH);
    end;
  end else
  begin
    case Axes of
      plXZ: if bmW> 0 then
              SetRect((pbW-bmW)/2,pbH-bmH,(pbW-bmW)/2+bmW,pbH) else
              SetRect((pbW-1000)/2,pbH-1000,(pbW-1000)/2+1000,pbH);
      plXY: if bmW> 0 then
              SetRect((pbW-bmW)/2,(pbH-bmH)/2,(pbW-bmW)/2+bmW,(pbH-bmH)/2 + bmH) else
              SetRect((pbW-1000)/2,(pbH-1000)/2,(pbW-1000)/2+1000,(pbH-1000)/2 + 1000);
...
dlgBGShift.fsSize.Value
dlgBGShift.fsHShift.Value
dlgBGShift.fsVShift.Value
