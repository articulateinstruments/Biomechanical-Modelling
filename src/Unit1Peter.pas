...
rotate doesn't work after zoom/scroll
  round wrong axis
...
begin
  ...
  if InputQuery('Rigid body','Rigid body name (empty name is non rigid)',s) then
    for a:=1 to NumAtoms do
      if Atoms[a].Selected then
        Atoms[a].Rigid:=s;
end;
  ...
  for a:=1 to NumAtoms do
    if Atoms[a].Rigid <> '' then
      if Atoms[a].Fixing <> chStatic then
        if pos('\'+Uppercase(Atoms[a].Rigid)+'\',s) = 0 then
        begin
          s:=s+'\'+Uppercase(Atoms[a].Rigid)+'\';
          case SymmetryAxes of
            plXY: ApplyXY(Uppercase(Atoms[a].Rigid));
            plXZ: ApplyXZ(Uppercase(Atoms[a].Rigid));
            plYZ: ApplyYZ(Uppercase(Atoms[a].Rigid));
          end;
        end;
end;
...
//                aImage.Picture.Create;
aImage.Picture.Bitmap.Create;
//                aImage.Picture.Graphic.Create;
                aImage.Picture.Picture.Width:=seUSImageWidth.Value;
                aImage.Picture.Picture.Height:=seUSImageHeight.Value;
                aImage.Width:=aImage.Picture.Picture.Width;
                aImage.Height:=aImage.Picture.Picture.Height;
