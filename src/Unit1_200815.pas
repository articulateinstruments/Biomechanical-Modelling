...
//    HasStruts: boolean;
  ...
  private
    ...
    function AtomsToStringOBJ(All: boolean): string;
...
//  ARect.Top:=round(zoom*Horizon);
    ...
    begin
      ...
      SaveToOBJFileString(AtomsToStringOBJ(true));
    end;
  end;
...
function TForm1.AtomsToStringOBJ(All: boolean): string;
var resultlen: integer;
  procedure AddToResult(s: string);
  begin
    if length(s)+resultlen > length(result) then
  ...
  procedure WriteAtomsOBJ(HasAtomsSelected: boolean);
  var a: TAtomRef;
  begin
    for a:=1 to NumAtoms do
      with Atoms[a] do
        if AtomSelected(a,HasAtomsSelected) then
          AddToResult('v'+#9+
            FixedPointStr(p.x,2)+#9+
            FixedPointStr(p.y,2)+#9+
            FixedPointStr(p.z,2)+
            #13#10);
  end;
  procedure WritePolygonsOBJ;
  var pg,e: integer;
  begin
    for pg:=0 to high(Polygons) do
    begin
      AddToResult('f'+#9);
      for e:=low(Polygons[pg].a) to high(Polygons[pg].a) do
        AddToResult(Inttostr(Polygons[pg].a[e])+#9);
      AddToResult(#13#10);
    end;
  end;
...
begin
  try
    ...
    result:='';
    resultlen:=0;
    AddToResult('# tongue.obj'+#13#10);
    AddToResult('#'+#13#10);
    AddToResult(#13#10+'g tongue'+#13#10);
    ...
    WriteAtomsOBJ(HasAtomsSelected);
    WritePolygonsOBJ;
    ...
    {this line only works for quads; otherwise use Area3D code for vector}
{
    ...
    f:=PolgonNormal(pg);
    area:=Length3D(f)/2;
    d:=GasConst*volume*pressure*(area/Totalsurfacearea)/area; {6 because it's usually a cube}
//    d:=GasConst*volume*pressure/6/area; {6 because it's usually a cube}
  ...
  var f: integer;
      v,Pressure,TotalSurfaceArea: single;
  begin
    ...
    for f:=0 to high(Polyhedron[ph].PosFace) do
      AddFacePressure(Polyhedron[ph].PosFace[f],-pressure,Polyhedron[ph].Volume,TotalSurfaceArea);
    for f:=0 to high(Polyhedron[ph].NegFace) do
      AddFacePressure(Polyhedron[ph].NegFace[f],+pressure,Polyhedron[ph].Volume,TotalSurfaceArea);
  end;
...
begin
  ...
  begin
    ...
    TimelineCurTime:=IntRange(ceil(TimelineCurTime+b),0,MaxTime);
    IsAtKeyframe;
    LoadTrackbars;
...
begin
  ...
  begin
    ...
    tmrKeyframe.enabled:=true;
    nextkeyframe:=0;
//    nextOutputTime:= TimelineCurTime;
  end else
  begin
    if not sbTimeline.Down then
      exit;
    if isAtkeyframe >= nextkeyframe  then
    begin
       nextkeyframe:= isAtkeyframe+ 1;
       if pnlRecordingMovie.Visible then
       begin
         SaveScreenShot('Mov',true);
         if count mod 5 = 0 then
            SaveToOBJFileString(AtomsToStringOBJ(true));   //only output every 5th frame given frame rate is 121Hz -> 20Hz
         pbTimelinePaint(nil);
         count:=count+1;
...
{    if nextOutputTime = TimelineCurTime then
    begin
       nextOutputTime := TimelineCurTime + 1000/dlgOptions.sePlaybackFPS.value);
       if pnlRecordingMovie.Visible then
       begin
         SaveScreenShot('Mov',true);
         pbTimelinePaint(nil);
...
begin
  ...
  for a:=1 to NumAtoms do
    if Atoms[a].Rigid <> nil then
      if Atoms[a].Fixing <> chStatic then
        for i:=0 to high(Atoms[a].Rigid) do
        if not(Atoms[a].Rigid[i] in s) then
        begin
          s:=s+[Atoms[a].Rigid[i]];
          case SymmetryAxes of
            plXY: ApplyXY(Atoms[a].Rigid[i]);
            plXZ: ApplyXZ(Atoms[a].Rigid[i]);
            plYZ: ApplyYZ(Atoms[a].Rigid[i]);
          end;
        end;
end;
...
object Significant2: TMenuItem
  Caption = 'Significant'
  object Significant3: TMenuItem
    Caption = 'Significant'
    OnClick = Significant3Click
  end
end
StringList: TStringList;
  StringList:=TStringList.Create;
  for m:=0 to high(muscle) do
    if (CurKeyframe >= 0) and Keyframe[CurKeyframe].posq[m].Significant then
      StringList.Add('+'+Before(muscle[m].Label1.Caption,'=')) else
      StringList.Add(Before(muscle[m].Label1.Caption,'='));
  CreateSubMenuList(Significant2,StringList,Significant3Click);
  StringList.Free;

    procedure Significant3Click(Sender: TObject);
procedure TForm1.Significant3Click(Sender: TObject);
begin
  if not InRange(TMenuItem(Sender).tag,0,high(muscle)) then
    exit;
  Keyframe[CurKeyframe].posq[TMenuItem(Sender).tag].Significant:=not Keyframe[CurKeyframe].posq[TMenuItem(Sender).tag].Significant;
  Keyframe0Significant;
  Modified:=true;
  LoadTrackbars;
  pbTimeline.Invalidate;
